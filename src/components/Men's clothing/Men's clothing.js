...
import "../Men's clothing/Men's clothing.css";
...
function Mensclothing(props) {
    ...
    const [Mensclothing, setProducts] = useState([]);

    useEffect(() => {
        fetch("https://fakestoreapi.com/products/category/men's clothing").then(res => res.json()).then(res => {
            setProducts(res)
        })
    }, []);
    ...
    return (
       ...
       {Mensclothing.length>0?Mensclothing.map((item,index)=>{
